...
#define TWO_16 65536
#define FORTY_MIL 40000000

uint_fast64_t next_numA(uint_fast64_t last) {
  uint_fast64_t mult = last * FACTOR_A;
  return mult % BIG_CONST;
}

uint_fast64_t next_numB(uint_fast64_t last) {
  uint_fast64_t mult = last * FACTOR_B;
  return mult % BIG_CONST;
}
...
int count_matches(uint_fast64_t a_num, uint_fast64_t b_num) {
  int count = 0;
  for (unsigned int i = 0; i < FORTY_MIL; i++) {
...
int main(void) {
  ...
  printf("Calculated %d, correct answer is 588\n", answer);
